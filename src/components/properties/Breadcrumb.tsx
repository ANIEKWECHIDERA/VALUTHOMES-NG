...
interface BreadcrumbItem {
  label: string;
  href?: string; // Optional, if no href, it’s the current page (no link)
}
...
export default function Breadcrumb({ items }: BreadcrumbProps) {
  return (
    <nav
      className="flex mb-5 sticky top-16 h-auto bg-soft-gray z-30 py-6"
        ...
        {items.map((item, index) => (
          <li key={index} className="inline-flex items-center">
            <div className="flex items-center">
              {index > 0 && (
                <svg
                  className="w-3 h-3 text-deep-navy-blue mx-1"
                  aria-hidden="true"
